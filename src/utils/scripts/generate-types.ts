...
const config: AppConfig = yaml.load(readFileSync(configPath, "utf-8")) as any
...
async function writeResult(
  ...
  const outpath = path.join(pathFromRoot, config.output_directory)
  const outfile = path.join(outpath, filename)

  await fs.mkdir(outpath, { recursive: true })

  console.log("Wrote to:", outfile, "\n")
  return fs.writeFile(outfile, text, "utf-8")
}
